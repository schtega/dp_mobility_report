...
def report_elements(dpmreport: "DpMobilityReport") -> dict:
    ...
    if dpmreport.evalu_analysis_selection_count is not None:
    ...
    if dpmreport.gaussian:
        ...
        if dpmreport.evalu_analysis_selection_count:
            deltai = dpmreport.delta / math.sqrt(dpmreport.evalu_analysis_selection_count)
        else:
            deltai = dpmreport.delta/math.sqrt((len(const.ELEMENTS) - len(dpmreport.analysis_exclusion)))
    else:
        deltai = None
...
def _get_eps(eps_factor: float, analysis_name: str, budget_split: dict, gaussian: bool = False) -> float:
    if eps_factor is None:
        ...
        return math.sqrt(budget_split[analysis_name]) * eps_factor
    else:
