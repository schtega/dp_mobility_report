...
class BenchmarkReport:
    ...
    def __init__(
        ...
        ds_report_path = os.path.join(config.REPORT_PATH, "berlin")
        shelve_path = os.path.join(
            ds_report_path, "gauss_" + str("Test") + "_count_" + str(16)
        )
        if os.path.exists(shelve_path + ".db"):
            os.remove(shelve_path + ".db")
        self.d = shelve.open(shelve_path)

        self.d[str(0)] = self.report_alternative.report
